...
6. **Five ansible_user accounts, five truths**:
   - **Windows workstations** (`group_vars/win10.yml`, `group_vars/win11.yml`): `xadmin:ConfigingInTheNameOf1!` — 36 hosts. The 2026-08-20 image refresh changed the shipped local account on the **workstation images only**. Child-group vars override the parent, which is what scopes this away from the servers.
   - **Windows servers** (`group_vars/windows.yml`): `simspace:Simspace1!Simspace1!` — the 9 hosts in `winserver2019`/`winserver2022`, unchanged by the refresh. Do **not** confuse this with `domain_admin` (`simspace`) in `group_vars/all.yml`, which the deploy *creates* during DC promotion; they share a password, which is precisely why they look interchangeable. `ansible_become_user` must track `ansible_user` in both files — `runas` has no separate become password, so a mismatch fails escalation on every Windows task.
